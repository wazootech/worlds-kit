...
test("encodeIri wraps IRIs in angle brackets", () => {
  assert.equal(
    encodeIri("https://example.com/foo"),
...
test("datasetIri and itemIri format correct IRIs", () => {
  assert.equal(
    datasetIri("my-world", "my-dataset"),
    "https://kit.wazoo.dev/world/my-world/dataset/my-dataset",
  );
  assert.equal(
    itemIri("my-world", "item-1"),
    "https://kit.wazoo.dev/world/my-world/item/item-1",
  );
...
test("queryBindings safely unwraps W3C SPARQL JSON and plain array formats", () => {
  ...
  assert.deepEqual(queryBindings({ results: { bindings: [{ id: "2" }] } }), [
    { id: "2" },
  ]);
...
test("createWorldsKitDataSource resolves dynamic worldId endpoint URLs", async () => {
  ...
  globalThis.fetch = (async (
    ...
    return new Response(
      JSON.stringify({ head: {}, results: { bindings: [] } }),
    );
