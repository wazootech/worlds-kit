...
export function subscribeToDataset(dataSource: WorldsKitDataSource, worldId: string, parentId: string, datasetId: string, onItems: (items: WorldsKitItem[]) => void, onError: (error: Error) => void): WorldsKitSubscription {
  ...
  if (dataSource.subscribe) return dataSource.subscribe<WorldsKitQueryResult<Record<string, unknown>>>(sparql, result => onItems(queryBindings(result).map(row => normalizeItem(row, worldId))), onError);
  let active = true;
  const controller = new AbortController();
  void dataSource.query<WorldsKitQueryResult<Record<string, unknown>>>(sparql, { signal: controller.signal }).then(result => { if (active) onItems(queryBindings(result).map(row => normalizeItem(row, worldId))); }).catch(error => { if (active && error.name !== "AbortError") onError(error); });
...
export function addDatasetItem(dataSource: WorldsKitDataSource, worldId: string, parentId: string, datasetId: string, orderIndex: number) {
  ...
  return dataSource.mutate(insert).then(() => ({ id }));
}
...
export function updateItem(dataSource: WorldsKitDataSource, worldId: string, id: string, updates: ItemUpdates) {
  ...
  return dataSource.mutate(`DELETE { ${deletePatterns} } INSERT { ${insertPatterns} } WHERE { ${optionalPatterns} }`);
}
...
export function subscribeToValue<T>(dataSource: WorldsKitDataSource, worldId: string, itemId: string, onValue: (value: T) => void, onError: (error: Error) => void): WorldsKitSubscription {
  ...
  if (dataSource.subscribe) return dataSource.subscribe<WorldsKitQueryResult<Record<string, unknown>>>(sparql, result => onValue((bindingValue(queryBindings(result)[0]?.value) ?? "") as T), onError);
  let active = true;
  const controller = new AbortController();
  void dataSource.query<WorldsKitQueryResult<Record<string, unknown>>>(sparql, { signal: controller.signal }).then(result => { if (active) onValue((bindingValue(queryBindings(result)[0]?.value) ?? "") as T); }).catch(error => { if (active && error.name !== "AbortError") onError(error); });
  return () => { active = false; controller.abort(); };
}
