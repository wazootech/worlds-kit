...
function itemIdFromIri(value: unknown, worldId: string) {
  ...
  const id =
    markerIndex >= 0
      ? rawId.slice(markerIndex + itemMarker.length)
...
function normalizeItem(
  ...
  const completed =
    row.completed === undefined
      ? undefined
      : normalizeTerm(row.completed) === "true";
  const orderIndex =
    row.orderIndex === undefined
      ? undefined
      : Number(normalizeTerm(row.orderIndex));
...
export function subscribeToDataset(
  ...
  parentId: string,
  datasetId: string,
  onItems: (items: WorldsKitItem[]) => void,
  onError: (error: Error) => void,
): WorldsKitSubscription {
  const sparql = itemQuery(worldId, parentId, datasetId);
  const processResult = (
    result: WorldsKitQueryResult<Record<string, unknown>>,
  ) => {
    const items = queryBindings(result).map((row) =>
      normalizeItem(row, worldId),
    );
    items.sort((a, b) => (a.orderIndex ?? 0) - (b.orderIndex ?? 0));
    onItems(items);
  };
  if (dataSource.subscribe)
    return dataSource.subscribe<WorldsKitQueryResult<Record<string, unknown>>>(
      sparql,
      processResult,
      onError,
      { worldId },
  ...
  void dataSource
    .query<WorldsKitQueryResult<Record<string, unknown>>>(sparql, {
      worldId,
      signal: controller.signal,
    })
    .then((result) => {
      if (active) processResult(result);
    })
    .catch((error) => {
...
export function updateDatasetItem(
  ...
  return updateItem(dataSource, worldId, id, updates);
}
...
export function updateItem(
  ...
  const deletePatterns = entries
    .map(([key], index) => `${subject} <${predicateIri(key)}> ?old${index} .`)
    .join(" ");
  const insertPatterns = entries
    .map(
      ([key, value]) =>
        `${subject} <${predicateIri(key)}> ${sparqlValue(value)} .`,
    )
    .join(" ");
  const optionalPatterns = entries
    .map(
      ([key], index) =>
        `OPTIONAL { ${subject} <${predicateIri(key)}> ?old${index} }`,
    )
    .join(" ");
  return dataSource.mutate(
    `DELETE { ${deletePatterns} } INSERT { ${insertPatterns} } WHERE { ${optionalPatterns} }`,
    { worldId },
  );
}

export function updateDatasetOrder(
  dataSource: WorldsKitDataSource,
  worldId: string,
  _parentId: string,
  _datasetId: string,
  ...
  return Promise.all(
    ids.map((id, index) =>
      updateItem(dataSource, worldId, id, { orderIndex: index }),
    ),
  );
}
...
export function subscribeToValue<T>(
  ...
  if (dataSource.subscribe)
    return dataSource.subscribe<WorldsKitQueryResult<Record<string, unknown>>>(
      sparql,
      (result) =>
        onValue((bindingValue(queryBindings(result)[0]?.value) ?? "") as T),
      onError,
      { worldId },
    );
  let active = true;
...
export function saveValue(
  ...
  return updateItem(dataSource, worldId, itemId, { value });
}

export function subscribeToDatasetItem(
  dataSource: WorldsKitDataSource,
  worldId: string,
  parentId: string,
  datasetId: string,
  id: string,
  onItem: (item: WorldsKitItem | null) => void,
  onError: (error: Error) => void,
): WorldsKitSubscription {
  return subscribeToDataset(
    ...
    parentId,
    datasetId,
    (items) => onItem(items.find((item) => item.id === id) ?? null),
    onError,
  );
}
